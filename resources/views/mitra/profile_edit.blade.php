...
<section class="space-y-6">
    ...
    <form method="POST" action="{{ route('mitra.profile.update') }}" enctype="multipart/form-data" class="rounded-[30px] bg-white p-8 shadow-[7px_12px_43px_0_rgba(0,0,0,0.14)]">
        @csrf
        ...
        <div class="grid gap-5">
            <div>
                ...
                <input type="text" name="nama" value="{{ $profile['nama'] }}" class="w-full rounded-[20px] bg-[#b0bac3]/40 px-5 py-4 text-xl" required>
            </div>
            <div>
                <label class="mb-2 block text-xl font-medium text-[#7c838a]">Nomor HP</label>
                <input type="text" name="phone" value="{{ $profile['phone'] ?? '' }}" class="w-full rounded-[20px] bg-[#b0bac3]/40 px-5 py-4 text-xl">
            </div>
            <div>
                <label class="mb-2 block text-xl font-medium text-[#7c838a]">Kota</label>
                <input type="text" name="location" value="{{ $profile['location'] ?? '' }}" class="w-full rounded-[20px] bg-[#b0bac3]/40 px-5 py-4 text-xl">
            </div>
            <div>
                <label class="mb-2 block text-xl font-medium text-[#7c838a]">Deskripsi Diri</label>
                <textarea name="bio" rows="4" class="w-full rounded-[20px] bg-[#b0bac3]/40 px-5 py-4 text-xl">{{ $profile['bio'] ?? '' }}</textarea>
            </div>
        </div>
        ...
        <footer class="mt-10 flex flex-wrap justify-center gap-4">
            ...
            <a href="{{ route('mitra.profile.show') }}" class="rounded-[10px] bg-[#49bef3] px-12 py-3 text-[22px] font-bold text-white">Kembali</a>
        </footer>
    </form>
</section>
@endsection
